...
class RepositoryStoragePersonImagesError(Exception):
    pass

class RepositoryStoragePersonImages:
    ...
    def search_one(self, person_image_path: EntityPersonImagePath) -> EntityPersonImage:
        path = self.storage_path / person_image_path.path
        if not path.exists():
            raise RepositoryStoragePersonImagesError(
                f"Person image not found: {path}")
        try:
            ...
            raise RepositoryStoragePersonImagesError(
                f"Error opening person image: {e}")
