...
class ServiceIdentifyPerson:
    ...
    @staticmethod
    def Identify(image: Image, camera_id: int = 0, view_id: int = 0) -> int:
        query_feature = ServiceIdentifyPerson.clip_reid_model.extract_feature(
            image,
            camera_id,
            view_id,
        )
        return ServiceIdentifyPerson.assign_id.assign(query_feature)
