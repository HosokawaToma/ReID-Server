...
# CERT_FILEとKEY_FILEが設定されていない場合はデフォルト値を設定
...
# 証明書が既に存在するか確認
if [ -f "${CERTS_PATH}/${CERT_FILE}" ] && [ -f "${CERTS_PATH}/${KEY_FILE}" ]; then
...
if [ -f "${CERTS_PATH}/${CERT_FILE}" ]; then
    CERT_PERM=$(stat -c "%a" "${CERTS_PATH}/${CERT_FILE}")
    if [ "${CERT_PERM}" != "644" ]; then
        sudo chmod 644 "${CERTS_PATH}/${CERT_FILE}"
        echo "✓ 証明書の権限を644に変更しました"
    else
        echo "✓ 証明書の権限は既に適切です"
    fi
else
    echo "エラー: 証明書ファイルが見つかりません: ${CERTS_PATH}/${CERT_FILE}"
    exit 1
fi

if [ -f "${CERTS_PATH}/${KEY_FILE}" ]; then
    KEY_PERM=$(stat -c "%a" "${CERTS_PATH}/${KEY_FILE}")
    if [ "${KEY_PERM}" != "644" ]; then
        sudo chmod 644 "${CERTS_PATH}/${KEY_FILE}"
        echo "✓ 秘密鍵の権限を644に変更しました"
    else
        echo "✓ 秘密鍵の権限は既に適切です"
    fi
else
    echo "エラー: 秘密鍵ファイルが見つかりません: ${CERTS_PATH}/${KEY_FILE}"
    exit 1
fi
...
echo "証明書ファイル:"
echo "  - 証明書: ${CERTS_PATH}/${CERT_FILE}"
echo "  - 秘密鍵: ${CERTS_PATH}/${KEY_FILE}"
echo ""

echo "証明書の有効期限:"
# Dockerを使って証明書の実際の有効期限を取得
CERT_NOT_BEFORE=$(run_openssl x509 -in "${CERT_FILE}" -noout -startdate 2>/dev/null | cut -d= -f2)
CERT_NOT_AFTER=$(run_openssl x509 -in "${CERT_FILE}" -noout -enddate 2>/dev/null | cut -d= -f2)
if [ -n "${CERT_NOT_BEFORE}" ] && [ -n "${CERT_NOT_AFTER}" ]; then
    ...
    CERT_DATE=$(stat -c %y "${CERTS_PATH}/${CERT_FILE}" 2>/dev/null | cut -d' ' -f1)
    echo "  - 作成日: ${CERT_DATE}"
